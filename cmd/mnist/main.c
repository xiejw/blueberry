...
//
// Thoughts: This creates a bunch of layers. It is good, but only works will for
// seq. For residual network, how to express?
...
main()
{
        ...
        error_t err = bbCreateLayers(
            ...
            &layers);
        if (err) {
                errDump("failed to create layers\n");
                goto cleanup;
        }
        ...
        err = bbCompileSeqModule(&ctx, p, x, y, layers, loss_layer, NULL, r);
        if (err) {
